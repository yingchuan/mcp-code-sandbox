...
class ExecutionTools:
    ...
    async def execute_code(self, session_id: str, code: str) -> Dict[str, Any]:
        """Execute Python code in the sandbox environment (direct method).
    ...
    async def install_package(self, session_id: str, package_name: str) -> Dict[str, Any]:
        """Install a Python package in the sandbox (direct method).
        ...
        try:
            ...
            raise RuntimeError(f"Error installing package: {str(e)}")

    async def create_run_close(self, code: str) -> Dict[str, Any]:
        """Create a sandbox, run code, and automatically close the sandbox in one operation (direct method).
        
        This is a convenience method that combines create_sandbox, execute_code, and close_sandbox
        ...
        try:
            ...
            # Extract parameters from interpreter_config
            backend_url = self.interpreter_config.get('backend_url')
            api_key = self.interpreter_config.get('api_key')
            
            # Pass parameters correctly to the create_interpreter method
